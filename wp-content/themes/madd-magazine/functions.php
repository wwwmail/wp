...
function get_posts_by_category($cat_id = 3, $posts_num = 4, $offset=0)
{
    return get_posts(
            [
...
function wp_infinitepaginate()
{
    ...
    # Load the posts
    query_posts(array('paged' => $paged, 'cat' => $_POST['cat_id']));
...
function get_infinity_posts($array_cat_ids = array(3, 4))
{
    $query = new WP_Query(array('category__in' => array(2, 3), 'posts_per_page' => 8));

    while ($query->have_posts()) {
        $query->the_post();
        get_template_part('template-parts/content', 'infinity-main-page');
    }
}

function get_infinity_posts_ajax($page = 1, $array_cat_ids = array(3, 4))
{
    $ppp = 8;
    $paged = $_POST['page_no'] ?? 0;
    $offset = 8;
    $page_offset = 0;
    ...
    $query = new WP_Query(array('category__in' => array(2, 3), 'posts_per_page' => $ppp, 'offset' => $page_offset));

    if ($query->have_posts()) {
        while ($query->have_posts()) {
            $query->the_post();
            get_template_part('template-parts/content', 'infinity-main-page');   //echo the_title(); // выведем заголовок поста
        }
    }
    die;
}

add_action('wp_ajax_infinite_scroll_home', 'get_infinity_posts_ajax');
add_action('wp_ajax_nopriv_infinite_scroll_home', 'get_infinity_posts_ajax');
